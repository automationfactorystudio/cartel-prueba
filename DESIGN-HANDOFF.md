# 8cc69840-bc9c-48ac-b626-563c44359dbd implementation handoff

This archive is the source of truth for turning the design into production code. Start from `cartel-synthwave.html`, then preserve the visual system, responsive behavior, and interactions found in the exported files.
...
## Source map
- Primary entry: `cartel-synthwave.html`
...
- A single self-contained `cartel-synthwave.html` is acceptable only when the export truly contains one user-facing screen and its CSS/JS are structured enough to extract tokens, components, states, and behavior.
...
1. Open `cartel-synthwave.html` and `DESIGN-MANIFEST.json`; identify every screen file, launcher/overview file, app module, and interaction before coding.
...
1. Inspect `cartel-synthwave.html` and `DESIGN-MANIFEST.json` first and identify reusable components before coding.
